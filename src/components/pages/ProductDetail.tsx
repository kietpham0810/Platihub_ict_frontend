    ...
    const fetchProductDetail = async () => {
      ...
      try {
        ...
        if (response.ok && data.status === 'success') {
          ...
          setError(data.message || t('productDetail.notFoundDesc'));
        }
      } catch {
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('productDetail.notFoundTitle')}</h2>
        <p className="text-gray-600 mb-6">{error || t('productDetail.notFoundDesc')}</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1200px] mx-auto px-4 py-3 text-sm text-gray-500">
          <Link to="/" className="hover:text-[#f26522] transition-colors">{t('productDetail.home')}</Link>
          <span className="mx-2">›</span>
          <Link
            to={`/san-pham?category=${encodeURIComponent(product.product_type)}`}
            className="hover:text-[#f26522] transition-colors"
          >
            {categoryLabel(product.product_type)}
          </Link>
          <span className="mx-2">›</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="md:w-7/12 p-8 flex flex-col">
            ...
            <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100 border-l-4 border-l-[#f26522]">
              <span className="text-gray-600 text-sm block mb-1">{t('productDetail.priceLabel')}</span>
              {product.is_price_visible === 1 && product.price ? (
                <span className="text-3xl font-extrabold text-[#f26522]">
                  {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(product.price)}
                </span>
              ) : (
                <span className="text-2xl font-extrabold text-[#f26522]">{t('productDetail.contactForPrice')}</span>
              )}
            </div>
            ...
            <div className="flex gap-4 mt-auto">
              <button onClick={() => setIsContactModalOpen(true)} className="flex-1 bg-gray-800 hover:bg-gray-900 text-white font-bold py-4 px-6 rounded-lg shadow-md transition-all flex items-center justify-center gap-2">
                <span>📞</span> {t('productDetail.callForAdvice')}
              </button>
            </div>
          </div>
        </div>
        ...
        {Object.keys(parsedSpecs).length > 0 && (
          <div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="bg-gray-50 px-8 py-5 border-b border-gray-200 flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-gray-800 uppercase tracking-wide">{t('productDetail.specsTitle')}</h2>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-gray-200 overflow-hidden animate-fade-in-up">
            <div className="flex justify-between items-center px-6 py-5 border-b border-gray-200">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{t('productDetail.contactModalTitle')}</h2>
                <p className="text-sm text-gray-500">{t('productDetail.contactModalSubtitle')}</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              <div className="rounded-3xl bg-slate-50 p-5 border border-slate-200">
                <p className="text-sm uppercase tracking-[0.25em] text-slate-500 font-semibold mb-2">{t('productDetail.addressLabel')}</p>
                <p className="text-base text-slate-700">159C De Tham Street, Cau Ong Lanh Ward, Ho Chi Minh City, Vietnam</p>
              </div>

              <div className="rounded-3xl bg-slate-50 p-5 border border-slate-200">
                <p className="text-sm uppercase tracking-[0.25em] text-slate-500 font-semibold mb-2">{t('productDetail.emailLabel')}</p>
              ...
              <div className="rounded-3xl bg-slate-50 p-5 border border-slate-200">
                <p className="text-sm uppercase tracking-[0.25em] text-slate-500 font-semibold mb-2">{t('productDetail.phoneLabel')}</p>
